...
public class CollosionTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (other != null)
        {
            ...
            //另一个球与墙壁的碰撞
            if ( (otherPos.x + 20) < otherSphere.radius || (20 - otherPos.x) < otherSphere.radius)
            {
                Debug.Log("墙壁碰撞发生！");
                otherSphere.currentV[0] *= -1;

                
            }

            if ( (otherPos.z + 10) < otherSphere.radius || (10 - otherPos.z) < otherSphere.radius)
            {
                Debug.Log("墙壁碰撞发生！");
                otherSphere.currentV[2] *= -1;


            }
        }

        //test if hit the right wall
        if( (pos[0] + 20) < 0.5 || (20 - pos[0]) < 0.5)
        {
            Debug.Log("墙壁碰撞发生！");
            preV[0] *= -1;
